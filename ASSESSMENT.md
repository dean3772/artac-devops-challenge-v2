# Assessment of the Inherited Codebase

This document reviews each issue or decision found across the Dockerfile, the CI/CD pipeline, and the Terraform configuration. Each finding lists what was found, a classification (Bug, Intentional Trade-off, or Needs Improvement), the contractor's reasoning from `DECISIONS.md` with my agreement or disagreement, and what I did and why.
...
**Contractor's reasoning:**
`DECISIONS.md` notes the deploy job is stubbed out and still needs wiring to real infrastructure. I agree a stub is acceptable during early setup, especially since the assignment does not require an actual AWS deployment, but a stub alone does not show the intended deployment design.

**What I did:**
Since I do not have AWS credentials or a real EC2 target, and the assignment does not require a live deploy, I replaced the TODO stub with a documented dry-run deployment template. The job runs and prints the intended real deployment flow without executing it: the required secrets, SSH key handling with a non-interactive host-key policy, an idempotent container replacement, and a post-deploy readiness check against `/ready`. The SSH key approach could also be replaced with a ready-made GitHub Actions SSH helper. In production I would prefer AWS SSM Session Manager over inbound SSH, as explained in Finding 17.

**Why:**
A documented, non-executing template communicates the full intended deployment flow and the production considerations behind it, while staying honest that no real deployment was performed and no money was spent. It is a clearer signal of deployment understanding than a placeholder, without overstating what was actually run.
...
**Contractor's reasoning:**
`DECISIONS.md` does not mention this dependency advisory. The contractor only mentioned relaxing Trivy to CRITICAL-only because scans were noisy and blocking the pipeline. This shows that the CRITICAL-only policy could hide actionable HIGH findings.
